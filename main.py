...
blacklist: List[str] = list(
    map(
        lambda l: l.replace("\r", "").replace("\n", "").split(":"),
        open("blacklists.txt", encoding="utf-8").readlines(),
    )
)
...
def clash_urls(html: str) -> List[str]:
    ...
    return (
        list(
            chain(
                *filter(
                    lambda y: len(y) > 0, map(lambda x: clash_reg.findall(x.text), html)
                )
            )
        )
        + clash_extra
...
def v2ray_urls(html: str) -> List[str]:
    ...
    return (
        list(
            chain(
                *filter(
                    lambda y: len(y) > 0, map(lambda x: v2ray_reg.findall(x.text), html)
                )
            )
        )
        + clash_extra
...
def merge_clash(configs: List[str]) -> str:
    ...
    """
    config_template: Dict[str, Any] = yaml.safe_load(
        open(clash_output_tpl, encoding="utf-8").read()
    )
    proxies: List[Dict[str, Any]] = []
    for i in range(len(configs)):
        tmp_config: Dict[str, Any] = yaml.safe_load(configs[i])
        if "proxies" not in tmp_config:
            continue
        for j in range(len(tmp_config["proxies"])):
            proxy: Dict[str, Any] = tmp_config["proxies"][j]
            if any(
                filter(
                    lambda p: p[0] == proxy["server"]
                    and str(p[1]) == str(proxy["port"]),
                    blacklist,
                )
            ):
                continue
            if any(
                filter(
                    lambda p: p["server"] == proxy["server"]
...
def main():
    ...
    with open(clash_output_file, mode="w", encoding="utf-8") as f:
        f.write(clash_merged)

    v2ray_merged: str = merge_v2ray(v2ray_configs)
    with open(v2ray_output_file, mode="w", encoding="utf-8") as f:
